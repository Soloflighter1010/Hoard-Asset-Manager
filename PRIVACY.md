...
| Product images | So the library loads quickly and works offline | `.cache/thumbs` in the Hoard folder |
...
- **Google Fonts,** which serves the typefaces on the tools' pages. Google receives your IP address and
  basic browser details when a page loads. See Google's privacy policy.
- **GitHub,** only when you open a link in a tool's footer or in the documentation.

That's all. The tools have no telemetry, no crash reporting and no automatic update checks.
